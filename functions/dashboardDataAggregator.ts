...
function generateNodeHeatmap(todayStats) {
  // Map sling scores to node colors
  // Anterior Sling: N2, N7, N11
  // Posterior Sling: N3, N9, N12
  // Lateral Sling: N1, N5, N6, N8, N10
...
function identifyImbalances(todayStats, avg7d) {
  ...
  if (anterior < 4) {
    alerts.push({
      sling: 'anterior',
      severity: 'high',
      message: 'Anterior Sling unterentwickelt. Mehr Dead Bugs & Pallof Presses empfohlen.',
      recommendation: 'Anterior-fokussierte Sessions'
    });
  }
  ...
  if (posterior < 4) {
    alerts.push({
      sling: 'posterior',
      severity: 'high',
      message: 'Posterior Sling unterentwickelt. Mehr Bird Dogs & Hinges empfohlen.',
      recommendation: 'Posterior-fokussierte Sessions'
    });
  }
  ...
  if (lateral < 4) {
    alerts.push({
      sling: 'lateral',
      severity: 'high',
      message: 'Lateral Sling unterentwickelt. Mehr Side Planks & Single-Leg Work empfohlen.',
      recommendation: 'Lateral-fokussierte Sessions'
    });
  }
  ...
  if (imbalance > 3) {
    alerts.push({
      sling: 'system',
      severity: 'medium',
      message: `Sling-Imbalance erkannt (${imbalance.toFixed(1)} Punkte). Asymmetrische Stabilität führt zu Verletzungsrisiko.`,
      recommendation: 'Balance-Arbeit zwischen den Slings'
    });
  }
...
function calculateHealthScore(todayStats) {
  ...
  // Health score = average score (0-100, scaled from 0-10) minus imbalance penalty
  const baseScore = avg * 10; // 0-100
  const imbalancePenalty = Math.min(20, imbalance * 5); // -0 to -20 based on imbalance
