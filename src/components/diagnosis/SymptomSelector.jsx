...
export default function SymptomSelector({ selectedRegion, selectedSymptoms, onSymptomSelect }) {
  ...
  // Get unique triggered chains from all selected symptoms
  const allTriggeredChains = selectedSymptoms && selectedSymptoms.length > 0
    ? [...new Set(selectedSymptoms.flatMap(s => s.triggered_chains || []))]
    : cluster.triggered_chains;
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <p className="text-sm text-slate-400 mb-4">
          Wähle alle Symptome, die auf dich zutreffen (Mehrfachauswahl möglich)
        </p>
        
        <div className="space-y-2">
          {cluster.symptoms.map((symptomObj, index) => {
            const isSelected = selectedSymptoms?.some(s => s.id === symptomObj.id) || false;
            return (
              <motion.button
                key={symptomObj.id}
              ...
              >
                <div className="flex-1">
                  ...
                  <span className={`text-xs mt-1 block font-medium ${isSelected ? 'text-slate-700' : 'text-slate-500'}`}>
